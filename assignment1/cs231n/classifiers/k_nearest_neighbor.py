...
class KNearestNeighbor(object):
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in range(num_test): # xrange
      for j in range(num_train): # xrange
        #####################################################################
        # TODO:                                                             #
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in range(num_test): # xrange
      ...
      #######################################################################
      dists[i,:] = np.sqrt(((self.X_train - X[i])**2).sum(axis=1)) # whole ith row. numpy 계산 방식?. col 기준으로 sum. axis=1? axis=0? 
      #######################################################################
      #                         END OF YOUR CODE                            #
      #######################################################################
    return dists
...
#     dists = np.sqrt(testtrain * -2 + test_sq.reshape(test_sq.shape[0], 1) + train_sq)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      [closest_y.append(self.y_train[idx]) for idx in idxs]
      #########################################################################
      # TODO:                                                                 #
