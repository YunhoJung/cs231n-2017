...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, dim in enumerate(hidden_dims):
            ...
            if i == 0:
                self.params['W' + str(i+1)] = np.random.randn(input_dim, dim) * weight_scale
            else:
                self.params['W' + str(i+1)] = np.random.randn(pre_dim, dim) * weight_scale
            if self.use_batchnorm:
                self.params['gamma' + str(i+1)] = np.ones(dim)
                self.params['beta' + str(i+1)] = np.zeros(dim)
            pre_dim = dim

        self.params['W' + str(self.num_layers)] = np.random.randn(pre_dim, num_classes) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if self.use_batchnorm and i != self.num_layers - 1:
                ...
                cache_dict[i+1]['b_cache'] = b_cache
            if i != self.num_layers - 1:
                scores, r_cache = relu_forward(scores)
                cache_dict[i+1]['r_cache'] = r_cache
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dout, dw, db = affine_backward(dout, cache_dict[self.num_layers].get('f_cache'))
        ...
        for i in reversed(range(1,self.num_layers)):
            ...
            # batchnorm
            if self.use_batchnorm:
                dout, dgamma, dbeta = batchnorm_backward(dout, cache_dict[i].get('b_cache'))
